...
func (r Router) AddReviewRouter(rg *gin.RouterGroup) {
	...
	reviewRouter.POST("/", middleware.JWTAuthMiddleWare(tokenmode.ACCESS_TOKEN, r.deps.redis), reviewCont.CreateReview)
	reviewRouter.GET("/", reviewCont.GetReviews)
	reviewRouter.GET("/:review_id", middleware.JWTAuthMiddleWare(tokenmode.ACCESS_TOKEN, r.deps.redis), reviewCont.GetReviewByID)
	...
	reviewRouter.PUT("/:review_id", middleware.JWTAuthMiddleWare(tokenmode.ACCESS_TOKEN, r.deps.redis), reviewCont.UpdateReview)
