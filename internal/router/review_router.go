...
func (r Router) AddReviewRouter(rg *gin.RouterGroup, mongoDB *mongo.Database) {
	...
	reviewRouter.GET("/", middleware.JWTAuthMiddleWare(tokenmode.ACCESS_TOKEN), cont.GetReviews)
	reviewRouter.GET("/:review_id", middleware.JWTAuthMiddleWare(tokenmode.ACCESS_TOKEN), cont.GetReviewByID)
	reviewRouter.GET("/seller/:seller_id", middleware.JWTAuthMiddleWare(tokenmode.ACCESS_TOKEN), cont.GetReviewsBySellerID)
